...
<header class="p-1 bg-dark text-white">
    <div class="container-fluid justify-content-start pb-2 px-3">
        <div class="d-flex flex-wrap align-items-center justify-content-start justify-content-lg-start mt-2">
            <a href="/" class="navbar-brand d-flex text-center align-items-center col-md-3 mb-2 mb-md-0 ps-md-5 ps-auto text-white text-decoration-none">
                <i class="bi bi-front fs-4 pe-1"></i> Presto
            </a>

            <ul class="nav col-12 col-lg-auto me-lg-auto mx-auto mb-2 mb-md-0">
                ...
                @auth
                    @if(Auth::user()->is_revisor)
                        <li>
                            <a href="{{route('revisor.home')}}" class="nav-link px-2 link-light">Revisor<i class="bi bi-house ps-2"></i>
                                @if (\App\Models\Announcement::ToBeRevisionedCount() == 0)
                                    <span class="badge rounded-pill bg-success">{{\App\Models\Announcement::ToBeRevisionedCount()}}</span>
                                @else
                                    <span class="badge rounded-pill bg-danger">{{\App\Models\Announcement::ToBeRevisionedCount()}}</span>                    
            ...
            <div class="col-md-4 text-end">
                @guest
                    <a class="btn btn-outline-light me-2" aria-current="page" href="{{route('login')}}">Login</a>
                    <a class="btn btn-warning" aria-current="page" href="{{route('register')}}">Registrati</a>
                @endguest
                @auth
                <div class="dropdown flex-shrink-0">
                    <a class="nav-link dropdown-toggle" href="#" id="dropdownLogout" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        <span class="text-light">{{__('ui.greatings')}} </span>{{Auth::user()->name}}
                    </a>
                    <ul class="dropdown-menu" aria-labelledby="dropdownLogout">
                        <li><a class="dropdown-item" href="#" onclick="event.preventDefault(), document.getElementById('logout').submit()">Logout</a></li>
                        <form method="POST" action="{{route('logout')}}" id="logout">
